...
def get_dict(word_set):
    ...
    for word in word_set:
        ...
        index += 1
    return word_index_dict, index_word_dict # the key of word_index is word, and the element of that is index; index_word change them   
...
def text_precess(train_text_loc, test_text_loc=None):
    ...
    if test_text_loc is None:
        sequence_len = len(max(train_tokens, key=len)) #token used in there ,is that a list of all sentences? test it. remember!!
    else:
    ...
    return sequence_len, len(word_index_dict) + 1  #sequence_len is the size of longest sentences; len(...)+1 is the number of words 
