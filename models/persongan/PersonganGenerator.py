...
model_filename = '/Preparation/models/english-bidirectional-distsim.tagger'
path_to_jar = '/Preparation/stanford-postagger.jar'
...
class Generator(object):
    def __init__(self, sequence_length, num_classes, num_vocabulary,
                 ...
                 goal_size, step_size, D_model, iw_dict, word_tag,max_index,LSTMlayer_num=1, l2_reg_lambda=0.0, learning_rate=0.01,drop_out=1):
        ...
        self.grad_clip = 5.0
        self.goal_out_size = goal_out_size #how many filters; may change for discriminator 
        ...
        self.epis = 0.65
        self.tem = 0.9
        self.NoTagTensor = tf.convert_to_tensor([0.0 for x in range(self.emb_dim)])
        
        self.WordTagArray = tf.TensorArray(dtype=tf.float32, size=0,
                                                          dynamic_size=True, infer_shape=True, clear_after_read=False)
        
        for word in self.word_tag.keys():
            self.WordTagArray = self.WordTagArray.write(int(word),self.word_tag[word])
        ...
        def _g_recurrence(i, x_t, h_tm1, h_tm1_manager, gen_x, goal, last_goal, real_goal, step_size,
                          gen_real_goal_array, gen_o_worker_array, input_x):
            ...
            h_t_Worker = self.g_worker_recurrent_unit(input_x, h_tm1)  # hidden_memory_tuple
            ...
            def fn(elem):
                input_x = tf.cond(tf.greater(elem,self.max_index),lambda: self.NoTagTensor,
                                  lambda: self.WordTagArray.read(elem))
                return input_x
            input_x = tf.map_fn(fn,next_token,dtype=tf.float32)
            input_x = tf.to_float(input_x)
            x_tp1 = tf.nn.embedding_lookup(self.g_embeddings, next_token)  # batch x emb_dim
            x_tp1_temp = x_tp1
            input_x = tf.concat([x_tp1_temp,input_x],1)
            with tf.control_dependencies([cur_sen]):
                gen_x = gen_x.write(i, next_token)  # index, batch_size
            return i + 1, x_tp1, h_t_Worker, h_t_manager, gen_x, goal, \
                ...
                , tf.cond(((i + 1) % step_size) > 0, lambda: real_goal,
                          lambda: real_sub_goal), step_size, gen_real_goal_array, gen_o_worker_array,input_x
        
        _, _, _, _, self.gen_x, _, _, _, _, self.gen_real_goal_array, self.gen_o_worker_array,input_x = control_flow_ops.while_loop(
            ...
            loop_vars=[tf.constant(0, dtype=tf.int32),
                       ...
                       gen_x, goal, tf.zeros([self.batch_size, self.goal_out_size]), self.goal_init, step_size,
                       gen_real_goal_array, gen_o_worker_array,tf.zeros([self.batch_size,2*self.emb_dim],dtype=tf.float32)],parallel_iterations=1) 
        ...
        def preTrain(i, x_t, g_predictions, h_tm1, input_x, h_tm1_manager, last_goal, real_goal, feature_array,
                     real_goal_array, sub_feature, all_sub_features, all_sub_goals,input_real_x):
            ## padding sentence by -1
            cur_sen = \
            ...
            sub_goal = self.g_manager_output_unit(h_t_manager)
            sub_goal = tf.nn.l2_normalize(sub_goal, 1)
            h_t_Worker = tf.cond(i > 0, lambda: self.g_worker_recurrent_unit(input_real_x, h_tm1),
            ...
            real_goal_array = tf.cond(((i) % step_size) > 0, lambda: real_goal_array,
                                      lambda: tf.cond((i) / step_size < self.sequence_length // step_size,
                                                      ...
                                                      lambda: real_goal_array))
            def fn_pre(elem):
                input_real_x = tf.cond(tf.greater(elem,self.max_index),lambda: self.NoTagTensor,
                                       lambda: self.WordTagArray.read(elem))
                return input_real_x
            temp_input_x = tf.transpose(input_x,[1,0])
            ttemp_input_x = temp_input_x[i-1]            
            input_real_x = tf.map_fn(fn_pre,ttemp_input_x,dtype=tf.float32)
            input_real_x = tf.to_float(input_real_x)
            x_tp1 = tf.cond(i > 0, lambda: ta_emb_x.read(i - 1),
                            lambda: x_t)
            x_tp1_temp = x_tp1
            input_real_x = tf.concat([x_tp1_temp,input_real_x],1)            
            ...
            return i + 1, x_tp1, g_predictions, h_t_Worker, input_x, h_t_manager, \
                   ...
                   feature_array, real_goal_array, sub_feature, all_sub_features, all_sub_goals, input_real_x

        _, _, self.g_predictions, _, _, _, _, _, self.feature_array, self.real_goal_array, self.sub_feature, self.all_sub_features, self.all_sub_goals,input_real_x = control_flow_ops.while_loop(
            ...
            real_goal_array, sub_feature, all_sub_features, all_sub_goals,tf.zeros([self.batch_size,2*self.emb_dim])),
    ...
    def rollout(self, input_x, given_num):
        ...
        def _g_recurrence_1(i, x_t, input_x, gen_x, h_tm1, h_tm1_manager, last_goal, real_goal, give_num, input_real_x,para):
            cur_sen = \
            tf.split(tf.concat([tf.split(input_x, [i, self.sequence_length - i], 1)[0], self.padding_array], 1),
            ...
            def fn_1(elem):
                input_real_x = tf.cond(tf.greater(elem,self.max_index),lambda: self.NoTagTensor,
                                       lambda: self.WordTagArray.read(elem))
                return input_real_x
            temp_input_x = tf.transpose(para,[1,0])
            ttemp_input_x = temp_input_x[i-1]
            input_real_x = tf.map_fn(fn_1,ttemp_input_x,dtype=tf.float32)
            input_real_x = tf.to_float(input_real_x)
            ...
            return i + 1, x_tp1, input_x, gen_x, h_t_Worker, h_t_manager, \
                   tf.cond(((i) % self.step_size) > 0, lambda: real_sub_goal,
                           lambda: tf.constant(0.0, shape=[self.batch_size, self.goal_out_size])), \
                   tf.cond(((i) % self.step_size) > 0, lambda: real_goal, lambda: real_sub_goal), give_num, input_real_x,para

        # When current index i >= given_num, start roll-out, use the output as time step t as the input at time step t+1
        def _g_recurrence_2(i, x_t, gen_x, h_tm1, h_tm1_manager, last_goal, real_goal, input_real_x):
            # with tf.device('/cpu:0'):
            cur_sen = tf.cond(i > 0, lambda:
            tf.split(tf.concat([tf.transpose(gen_x.stack(), perm=[1, 0]), self.padding_array], 1),
            ...
            h_t_Worker = self.g_worker_recurrent_unit(input_real_x, h_tm1)  # hidden_memory_tuple
            o_t_Worker = self.g_worker_output_unit(h_t_Worker)  # batch x vocab , logits not prob
            ...
            def fn_2(elem):
                input_real_x = tf.cond(tf.greater(elem,self.max_index),lambda: self.NoTagTensor,
                                       lambda: self.WordTagArray.read(elem))
                return input_real_x          
            input_real_x = tf.map_fn(fn_2,next_token,dtype=tf.float32)
            input_real_x = tf.to_float(input_real_x)           
            ...
            return i + 1, x_tp1, gen_x, h_t_Worker, h_t_manager, \
                   tf.cond(((i) % self.step_size) > 0, lambda: real_sub_goal,
                           lambda: tf.constant(0.0, shape=[self.batch_size, self.goal_out_size])), \
                   tf.cond(((i) % self.step_size) > 0, lambda: real_goal, lambda: real_sub_goal),input_real_x

        i, x_t, _, gen_for_reward, h_worker, h_manager, self.last_goal_for_reward, self.real_goal_for_reward, given_num,input_real_x,para = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4, _5, _6, _7, given_num,_8,_9: i < given_num + 1,
            ...
            given_num,tf.zeros([self.batch_size,2*self.emb_dim]),input_x), parallel_iterations=1)  ##input groud-truth

        _, _, gen_for_reward, _, _, _, _,input_real_x = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4, _5, _6,_7: i < self.sequence_length + 1,
            body=_g_recurrence_2,
            loop_vars=(
            i, x_t, gen_for_reward, h_worker, h_manager, self.last_goal_for_reward, self.real_goal_for_reward,tf.zeros([self.batch_size,2*self.emb_dim])),
    ...
    def create_Worker_recurrent_unit(self, params):
        with tf.variable_scope('Worker'):
            ...
            self.Wi_worker = tf.Variable(tf.random_normal([2*self.emb_dim, self.hidden_dim], stddev=0.1))
            ...
            self.Wf_worker = tf.Variable(tf.random_normal([2*self.emb_dim, self.hidden_dim], stddev=0.1))
            ...
            self.Wog_worker = tf.Variable(tf.random_normal([2*self.emb_dim, self.hidden_dim], stddev=0.1))
            ...
            self.Wc_worker = tf.Variable(tf.random_normal([2*self.emb_dim, self.hidden_dim], stddev=0.1))
    ...
    def create_Manager_recurrent_unit(self, params):
        with tf.variable_scope('Manager'):
            # Weights and Bias for input and hidden tensor
            self.Wi = tf.Variable(tf.random_normal([self.num_filters_total+2*self.dis_emb_dim, self.hidden_dim], stddev=0.1))
            ...
            self.Wf = tf.Variable(tf.random_normal([self.num_filters_total+2*self.dis_emb_dim, self.hidden_dim], stddev=0.1))
            ...
            self.Wog = tf.Variable(tf.random_normal([self.num_filters_total+2*self.dis_emb_dim, self.hidden_dim], stddev=0.1))
            ...
            self.Wc = tf.Variable(tf.random_normal([self.num_filters_total+2*self.dis_emb_dim, self.hidden_dim], stddev=0.1))
