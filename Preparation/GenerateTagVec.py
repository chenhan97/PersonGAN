...
def Text_to_tag(Readfile,file):
    remove(sys.path[0]+'/save/data/'+file)
...
def GetTagVec(author_name):
    Text_to_tag(sys.path[0]+"/save/data/"+author_name,sys.path[0]+"/save/data/"+author_name+"_tag")
    sentences = MySentences(sys.path[0]+"/save/data/"+author_name+"_tag")
    model = gensim.models.word2vec.Word2Vec(sentences,size = 100,hs = 1,min_count=5,window =5)
    model.save(sys.path[0]+"/save/tagvec_model")
